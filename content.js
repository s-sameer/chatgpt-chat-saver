...
const observer = new MutationObserver(() => {
  const sidebar = document
    .querySelector('nav a[title="ChatGPT"]')
    ?.closest("div");
...
function createSavedChatsButton(sidebar) {
  ...
  savedChatsButton.className =
    "text-sm grow overflow-hidden text-ellipsis whitespace-nowrap text-token-text-primary";
  ...
  savedChatsButton.addEventListener("mouseleave", () => {
    savedChatsButton.style.backgroundColor =
...
function resetChatState(href) {
  ...
  const originalChatItem = document
    .querySelector(`li[data-testid^="history-item-"] a[href="${relativeHref}"]`)
    ?.closest("li");
  if (originalChatItem) {
    const originalSaveIconWrapper =
      originalChatItem.querySelector(".save-icon-wrapper");
...
function loadSavedChats() {
  ...
  Object.entries(savedChats).forEach(([href, chatHTML]) => {
    ...
    chatItem
      .querySelector(".save-icon-wrapper")
...
function removeSavedChat(href, savedChatsList) {
  ...
  div.innerHTML = `
      <div class="dialog-wrapper scale p-2 bg-gray-800">
        <div class="flex items-center pb-2 border-b border-gray-700 gap-2 mb-2 ">
          <span class="del-btn ">${deleteIcon}</span>
          <p class="text-gray-200 font-semibold  text-lg">Are you sure?</p>
        </div>
          <span class="text-sm leading-[25px] text-gray-400 "> Do you really want to delete this saved chats? This process can't be undone. </span>
  ...
  // - shake if clicked outside the dialog
  ...
  deleteButton.addEventListener("click", () => {
    const savedChatItem = Array.from(
      savedChatsList.querySelectorAll("li")
    ).find((item) => item.querySelector("a")?.href === href);
