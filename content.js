...
function createConfirmationDialog(savedChatItem) {
    const div = document.createElement('div');
    div.className = 'alert-overlay';
    div.innerHTML = `
      <div class="dialog-wrapper max-w-md w-full rounded-2xl shadow-xl flex flex-col overflow-hidden focus:outline-none  scale bg-token-main-surface-primary text-start">
       <div class="px-4 pb-4 pt-5 sm:p-6 flex items-center justify-between border-b border-black/10 dark:border-white/10"><div class="flex"><div class="flex items-center"><div class="flex grow flex-col gap-1"><h2 id="radix-:r6i:" class="text-lg font-semibold leading-6 text-token-text-primary">Delete chat?</h2></div></div></div></div>
       <div class="flex-grow overflow-y-auto p-4 sm:p-6">
        This will delete <strong>${savedChatItem.innerText}</strong>.
        <div class="mt-2 text-sm text-token-text-tertiary">
          To clear any memories from this chat, visit your 
          <a id="personalization-settings" class="underline" data-discover="true" href="#settings/Personalization">settings</a>.
        </div>
        <div class="flex mt-4 gap-3 sm:mt-4 mt-5 flex-col sm:flex-row-reverse">
          <button class="btn btn-danger relative" id="delete-btn">
            <div class="flex items-center justify-center">Delete</div>
          </button>
          <button class="btn relative btn-secondary" id="close-btn">
...
function closeDialog(alertOverlay) {
    if (alertOverlay) alertOverlay.remove();
}
function handleDelete(savedChatItem, href, alertOverlay) {
    if (savedChatItem) {
        savedChatItem.remove();
        removeChatFromLocalStorage(href); 
        resetChatState(href);           
        closeDialog(alertOverlay);
    }
}
function attachDialogListeners(alertOverlay, savedChatItem, href) {
    const closeButton = alertOverlay.querySelector('#close-btn');
    const deleteButton = alertOverlay.querySelector('#delete-btn');
    const personalizationSettingsBtn = alertOverlay.querySelector('#personalization-settings');
    
    closeButton.addEventListener('click', () => closeDialog(alertOverlay));
    personalizationSettingsBtn.addEventListener('click', () => closeDialog(alertOverlay));
    alertOverlay.addEventListener('click', (e) => {
        if (e.target.classList.contains('alert-overlay') && !e.target.closest('.dialog-wrapper')) {
            closeDialog(alertOverlay);
        }
    });
    deleteButton.addEventListener('click', () => handleDelete(savedChatItem, href, alertOverlay));
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') closeDialog(alertOverlay);
    });
}
function removeSavedChat(href, savedChatsList) {
    ...
    if (!savedChatItem) return;
    const alertOverlay = createConfirmationDialog(savedChatItem);
    document.body.appendChild(alertOverlay);
    attachDialogListeners(alertOverlay, savedChatItem, href);
}
